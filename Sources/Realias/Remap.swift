...
enum Remap {
	...
	static var cloudStorage: String { (home as NSString).appendingPathComponent("Library/CloudStorage") }
	...
	static func localRoots() -> [String] {
		...
		if FileManager.default.fileExists(atPath: cloudStorage, isDirectory: &isDirectory), isDirectory.boolValue {
	...
	static func remap(_ path: String) throws -> String {
		...
		let matching = roots.filter { normalized(($0 as NSString).lastPathComponent) == normalized(foreignRoot) }
